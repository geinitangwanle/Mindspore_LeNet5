...
# 在MindSpore中冻结参数的正确方法
print("冻结除全连接层外的所有参数...")
# 只获取全连接层的可训练参数
trainable_params = []
for name, param in network.parameters_and_names():
    if "fc" in name:  # 只有全连接层的参数可训练
        trainable_params.append(param)
        print(f"可训练参数: {name}")
    
print(f"可训练参数数量: {len(trainable_params)}")

# 定义优化器和损失函数，只更新全连接层参数
opt = nn.Momentum(params=trainable_params, learning_rate=lr, momentum=momentum)
...
best_ckpt_path = "./BestCheckpoint/resnet50-best3.ckpt"
